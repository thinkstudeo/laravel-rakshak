...
<div class="w-full bg-white flex flex-col justify-center items-center h-full pt-24">
    ...
    <div class="my-6 lg:w-1/2 lg:max-w-md">
        <a 
            href="{{ route('guardian.roles.index') }}" 
    ...
    <div class="lg:w-1/2 lg:max-w-md flex flex-col shadow-md border-t-8 border-purple rounded">
        <form action="{{ route('guardian.roles.store') }}" method="POST" class="p-8 bg-grey-lighter">
            @if (session('status'))
            ...
            <div class="mb-10">
                <h1 class="text-2xl font-bold mb-2">{{ __('Create New Role') }}</h1>
            </div>

            @include('guardian::roles._form')
